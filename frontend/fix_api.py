...
export const nutritionAPI = {
  getRecipes: async ({ goal, restrictions = '' }) => {
    const response = await api.get('/api/nutrition/recipes', {
      params: { goal, restrictions: Array.isArray(restrictions) ? restrictions.join(',') : restrictions },
    });
    return response.data;
  },
  getMealPlan: async ({ goal, restrictions = '', days = 7 }) => {
    const response = await api.get('/api/nutrition/meal-plan', {
      params: { goal, restrictions: Array.isArray(restrictions) ? restrictions.join(',') : restrictions, days },
    });
    return response.data;
  },
  getHealthySwaps: async ({ craving }) => {
    const response = await api.get('/api/nutrition/healthy-swaps', { params: { craving } });
    return response.data;
  },
};
